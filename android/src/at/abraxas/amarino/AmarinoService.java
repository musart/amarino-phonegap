...
import com.musart.amarino_android.R;
...
public class AmarinoService extends Service {
	...
	private void forwardDataToArduino(Intent intent){
		...
		if (pluginId == -1) {
			...
			try {
				sendData(address, message.getBytes("ISO-8859-1"), callbackId);
				...
				sendData(address, message.getBytes(), callbackId);
			}
		}
		else {
			...
			if (devices != null && devices.size() != 0){
				for (BTDevice device : devices){
					...
					sendData(device.getAddress(), message.getBytes(), "musart");
				}
			}
			else {
	...
	protected void connect(final String address, final String callbackId){
		...
		localDevice.init(this, new ReadyListener() {
			...
			public void ready() {
				...
				localDevice.destroy();
				new ConnectThread(device, callbackId).start();
			}
		});
	...
	public void sendData(final String address, byte[] data, String callbackId){
		ConnectedThread ct = connections.get(address);
		ct.setCallbackId(callbackId);
		if (ct != null)
			ct.write(data);
	}
	...
	private void sendConnectionEstablished(String address, String callbackId){
		String info = getString(R.string.service_connected_to, address);
		Logger.d(TAG, info);

		sendBroadcast(new Intent(AmarinoIntent.ACTION_CONNECTED)
			.putExtra(AmarinoIntent.EXTRA_DEVICE_ADDRESS, address)
			.putExtra(AmarinoIntent.EXTRA_PHONEGAP_CALLBACK_ID, callbackId));
	...
	private class ConnectThread extends Thread {
	    ...
	    public ConnectThread(RemoteDevice device, String callbackId) {
	        mDevice = device;
	    ...
	    public void run() {
	      	try {
	    		...
	    		if (!isPaired){
			        ...
			        manageConnectedSocket(mSocket, mCallbackId);
	    		}
			}
	    ...
	    private void manageConnectedSocket(BtSocket socket, String callbackId){
	    	Logger.d(TAG, "connection established.");
	    	// pass the socket to a worker thread
	    	String address = mDevice.getAddress();
	    	ConnectedThread t = new ConnectedThread(socket, address, callbackId);
	...
	private class ConnectedThread extends Thread {
	    ...
	    public ConnectedThread(BtSocket socket, String address, String callbackId) {
	        mSocket = socket;
	    ...
	    public void run() {
	        ...
	        sendConnectionEstablished(mAddress, mCallbackId);
...
//					}
	    ...
	    private void forwardDataToOtherApps(String msg){
	    	...
	    	intent.putExtra(AmarinoIntent.EXTRA_PHONEGAP_CALLBACK_ID, mCallbackId);
